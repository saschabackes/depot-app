...
function SuperAdminSection() {
  ...
  async function loadAll() {
    ...
    setError('')
    try {
      const [u, s, ov] = await Promise.all([
        superListUsers(),
        superStats().catch(() => null),
        superActivityOverview().catch(() => null),
      ])
      setUsers(Array.isArray(u) ? u : [])
      setStats(s)
      if (ov) { ov.totalUsers = Array.isArray(u) ? u.length : 0 }
      setOverview(ov)
      markSeen()
    } catch (e) {
      setError(e.message)
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="card px-3 py-3 text-center">
              <div className="text-2xl font-bold text-primary-600 dark:text-primary-400">{overview?.active7 ?? '—'}</div>
              <div className="text-xs text-gray-400 font-medium mt-0.5">Aktiv (7 Tage)</div>
            </div>
            <div className="card px-3 py-3 text-center">
              <div className="text-2xl font-bold text-primary-600 dark:text-primary-400">{overview?.active30 ?? '—'}</div>
              <div className="text-xs text-gray-400 font-medium mt-0.5">Aktiv (30 Tage)</div>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2">
            <div className="card px-3 py-3 text-center">
              <div className="text-lg font-bold text-gray-900 dark:text-gray-100">{users.length}</div>
              <div className="text-xs text-gray-400 font-medium mt-0.5">Nutzer</div>
            </div>
            <div className="card px-3 py-3 text-center">
              <div className="text-lg font-bold text-gray-900 dark:text-gray-100">{stats?.households ?? '—'}</div>
              <div className="text-xs text-gray-400 font-medium mt-0.5">Haushalte</div>
            </div>
            <div className="card px-3 py-3 text-center">
              <div className="text-lg font-bold text-gray-900 dark:text-gray-100">{stats?.spices ?? '—'}</div>
          ...
          {overview?.topUsers?.length > 0 && (
            <div>
              <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Aktivste Nutzer (30 Tage)</h4>
                    ...
                    <div key={u.userId} className="flex items-center gap-3 px-3 py-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-0.5 h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                          <div className="h-full bg-primary-500 rounded-full" style={{ width: `${(u.count / maxCount) * 100}%` }} />
                        </div>
                      </div>
                      <span className="text-xs font-semibold text-gray-500 flex-none">{u.count}</span>
                    </div>
                  )
                })}
              </div>
          ...
          {overview?.feed?.length > 0 && (
            <div>
              <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Letzte Aktivitäten</h4>
              <div className="card divide-y divide-gray-100 dark:divide-gray-700 max-h-72 overflow-y-auto">
                {overview.feed.map((a, i) => (
                  <div key={i} className="flex items-start gap-2 px-3 py-2 text-xs">
                    <span className="text-gray-400 flex-none w-20">{fmtDateTime(a.created_at)}</span>
                    <div className="flex-1 min-w-0">
                      <span className="font-semibold text-gray-700 dark:text-gray-200">{ACTION_LABELS[a.action] || a.action}</span>
                      {a.target_name && <span className="text-gray-500"> · {a.target_name}</span>}
                      {a.user_name && <span className="text-gray-400 block truncate">{a.user_name}</span>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {loading && !overview && (
            <p className="text-sm text-gray-400 text-center py-4">Lade Übersicht…</p>
          )}
      ...
      {/* ── Nutzer-Tab ── */}
      {mainTab === 'users' && (<div className="space-y-4">
